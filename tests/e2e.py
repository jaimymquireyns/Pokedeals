...
def main():
    ...
    time.sleep(1)
    mock = Mock()
    for f in mock.db["forecasts"]:
        if f["product_id"] == "sv08-100":
            f["confidence"] = "laag"
    errors = []
    try:
        with sync_playwright() as pw:
            ...
            check(page.locator(".tag.grof").count() == 1, "label 'grof' bij een grove schatting")
            check("Nog grove schattingen" not in page.inner_text("#app"), "geen waarschuwing als de meeste schattingen wel goed zijn")
            ...
            check("Near Mint vanaf" in page.inner_text(".nmbox") and "38,00" in page.inner_text(".nmbox"), "laagste NM-prijs getoond: " + page.inner_text(".nmbox").replace("\n", " "))
            check("72%" in page.inner_text(".chance") and "6%" in page.inner_text(".chance"), "kansbalk toont 72% en 6%")
            check("Kans binnen 30 dagen" in page.inner_text(".sec"), "standaardperiode is 30 dagen")
            page.locator(".seg.periods button", has_text="24m").click()
            page.wait_for_selector("text=Kans binnen 2 jaar")
            check("Nog niet berekend" in page.inner_text(".sec"), "lange periode zonder data toont nette melding")
            page.locator(".seg.periods button", has_text="30d").click()
            page.wait_for_selector("text=72%")
            ...
            page.locator("input[type=password]").fill("verkeerd-wachtwoord")
            page.locator(".login .cta").click()
            page.wait_for_selector(".err:not(:empty)")
            check("klopt niet" in page.inner_text(".err"), "inloggen zonder account geeft melding")
            page.get_by_role("button", name="Nog geen account? Maak er een").click()
            page.locator("input[type=email]").fill("ik@example.nl")
            page.locator("input[type=password]").fill("kort")
            page.locator(".login .cta").click()
            check("minstens 8" in page.inner_text(".err"), "te kort wachtwoord wordt geweigerd")
            page.locator("input[type=password]").fill("goed-wachtwoord-1")
            page.locator(".login .cta").click()
            ...
            print("Watchlist: volgen vanaf kaartdetail")
            page.goto(base + "#/detail/sv08-100")
            page.wait_for_selector(".chance")
            page.get_by_role("button", name="Volgen", exact=True).click()
            page.wait_for_selector("text=Wordt gevolgd")
            check(len(mock.db["watch_items"]) == 1 and mock.db["watch_items"][0]["product_id"] == "sv08-100", "kaart toegevoegd aan watchlist")
            page.get_by_role("button", name="Wordt gevolgd").click()
            page.wait_for_selector("text=Volgen")
            check(len(mock.db["watch_items"]) == 0, "kaart weer verwijderd via dezelfde knop")
            page.get_by_role("button", name="Volgen", exact=True).click()
            page.wait_for_selector("text=Wordt gevolgd")
            page.goto(base + "#/detail/sv03-125")
            page.wait_for_selector(".chance")
            page.get_by_role("button", name="Volgen", exact=True).click()
            page.wait_for_selector("text=Wordt gevolgd")
            check(len(mock.db["watch_items"]) == 2, "tweede kaart toegevoegd")

            print("Watchlist: tabblad, mappen, sorteren")
            page.goto(base + "#/watchlist")
            page.wait_for_selector(".wrow")
            check(page.locator(".wrow").count() == 2, "2 gevolgde kaarten zichtbaar")
            page.screenshot(path=str(SHOTS / "watchlist.png"), full_page=True)
            page.locator(".fchip.add").click()
            page.wait_for_selector(".sheetin input[type=text]")
            page.locator(".sheetin input[type=text]").fill("Interessant")
            page.get_by_role("button", name="Map maken").click()
            page.wait_for_selector(".fchip:not(.add)", state="attached")
            check(any(f["name"] == "Interessant" for f in mock.db["watch_folders"]), "map aangemaakt")
            page.locator(".morebtn").first.click()
            page.wait_for_selector(".chkrow input")
            page.locator(".chkrow input").check()
            page.wait_for_timeout(300)
            check(len(mock.db["watch_folder_items"]) == 1, "kaart aan map gekoppeld")
            page.keyboard.press("Escape")
            page.get_by_role("button", name="Interessant", exact=True).click()
            page.wait_for_selector(".wrow")
            check(page.locator(".wrow").count() == 1, "filter op map toont alleen gekoppelde kaart")
            page.get_by_role("button", name="Alles", exact=True).click()
            page.locator(".sortb").click()
            page.get_by_role("button", name="Grootste kans op stijging").click()
            names_w = page.locator(".wrow .name").all_inner_texts()
            check(len(names_w) == 2, "sorteren toegepast, 2 rijen: " + str(names_w))
            page.locator(".morebtn").first.click()
            page.wait_for_selector("text=Verwijderen uit watchlist")
            page.get_by_role("button", name="Verwijderen uit watchlist").click()
            page.wait_for_timeout(300)
            check(len(mock.db["watch_items"]) == 1, "verwijderd uit watchlist")
